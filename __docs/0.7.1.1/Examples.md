...
cd druid
git fetch --tags
git checkout druid-0.7.1.1
mvn clean package
```

### Downloading the DSK (Druid Standalone Kit)

[Download](http://static.druid.io/artifacts/releases/druid-0.7.1.1-bin.tar.gz) a stand-alone tarball and run it:

``` bash
tar -xzf druid-0.7.1.1-bin.tar.gz
cd druid-0.7.1.1
```
