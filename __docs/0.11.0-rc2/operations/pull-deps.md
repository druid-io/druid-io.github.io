...
Version to use for extension coordinate that doesn't have a version information. For example, if extension coordinate is `io.druid.extensions:mysql-metadata-storage`, and default version is `0.11.0-rc2`, then this coordinate will be treated as `io.druid.extensions:mysql-metadata-storage:0.11.0-rc2`
...
Suppose you want to download ```druid-rabbitmq```, ```mysql-metadata-storage``` and ```hadoop-client```(both 2.3.0 and 2.4.0) with a specific version, you can run `pull-deps` command with `-c io.druid.extensions:druid-examples:0.11.0-rc2`, `-c io.druid.extensions:mysql-metadata-storage:0.11.0-rc2`, `-h org.apache.hadoop:hadoop-client:2.3.0` and `-h org.apache.hadoop:hadoop-client:2.4.0`, an example command would be:

```
java -classpath "/my/druid/lib/*" io.druid.cli.Main tools pull-deps --clean -c io.druid.extensions:mysql-metadata-storage:0.11.0-rc2 -c io.druid.extensions.contrib:druid-rabbitmq:0.11.0-rc2 -h org.apache.hadoop:hadoop-client:2.3.0 -h org.apache.hadoop:hadoop-client:2.4.0
```
...
└── mysql-metadata-storage
    ...
    └── mysql-metadata-storage-0.11.0-rc2.jar
```
...
Note that if you specify `--defaultVersion`, you don't have to put version information in the coordinate. For example, if you want both `druid-rabbitmq` and `mysql-metadata-storage` to use version `0.11.0-rc2`,  you can change the command above to

```
java -classpath "/my/druid/lib/*" io.druid.cli.Main tools pull-deps --defaultVersion 0.11.0-rc2 --clean -c io.druid.extensions:mysql-metadata-storage -c io.druid.extensions.contrib:druid-rabbitmq -h org.apache.hadoop:hadoop-client:2.3.0 -h org.apache.hadoop:hadoop-client:2.4.0
```
