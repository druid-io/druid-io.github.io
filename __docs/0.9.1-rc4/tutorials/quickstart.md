...
```bash
curl -O http://static.druid.io/artifacts/releases/druid-0.9.1-rc4-bin.tar.gz
tar -xzf druid-0.9.1-rc4-bin.tar.gz
cd druid-0.9.1-rc4
```
...
With Zookeeper running, return to the druid-0.9.1-rc4 directory. In that directory, issue the command:
...
this task, POST it to Druid in a new terminal window from the druid-0.9.1-rc4 directory:
