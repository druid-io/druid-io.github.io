...
```bash
curl -O http://static.druid.io/artifacts/releases/druid-0.12.3-rc1-bin.tar.gz
tar -xzf druid-0.12.3-rc1-bin.tar.gz
cd druid-0.12.3-rc1
```
...
```bash
curl -O http://druid.io/docs/0.12.3-rc1/tutorials/tutorial-examples.tar.gz
...
With Zookeeper running, return to the druid-0.12.3-rc1 directory. In that directory, issue the command:
...
All persistent state such as the cluster metadata store and segments for the services will be kept in the `var` directory under the druid-0.12.3-rc1 package root. 
...
From the druid-0.12.3-rc1 directory:
